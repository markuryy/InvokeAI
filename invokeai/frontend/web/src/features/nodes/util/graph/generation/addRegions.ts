...
export const addRegions = async ({
  ...
  const isChroma = model.base === 'chroma';
  ...
  for (const region of validRegions) {
    ...
    if (region.positivePrompt) {
      ...
      } else if (posCond.type === 'chroma_text_encoder') {
        for (const edge of g.getEdgesTo(posCond, ['t5_encoder', 'mask'])) {
          const clone = deepClone(edge);
          clone.destination.node_id = regionalPosCond.id;
          g.addEdgeFromObj(clone);
        }
    ...
    if (region.negativePrompt) {
      ...
      } else if (negCond.type === 'chroma_text_encoder') {
        for (const edge of g.getEdgesTo(negCond, ['t5_encoder', 'mask'])) {
          const clone = deepClone(edge);
          clone.destination.node_id = regionalNegCond.id;
          g.addEdgeFromObj(clone);
        }
